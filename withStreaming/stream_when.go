...
// TestStreamWhen_Internal demonstrates the SDK streaming feature for when by
// creating a Rendering Context with both Model and Error channels.
func TestStreamWhen_Internal[T whenType](mode ...string) {
	_ = mode
	logger.Info("Entering TestStreamWhen_Internal")
	...
	logger.Info("Leaving TestStreamWhen_Internal")
}

type whenType interface {
	// EXISTING_CODE
	// EXISTING_CODE
}

// TestStreamWhen calls into _Internal
func TestStreamWhen() {
	// EXISTING_CODE
	TestStreamWhen_Internal[*types.NamedBlock]()
	// EXISTING_CODE
}
...
// WhenCount ([]types.Count, *types.MetaData, error) {
